  ...
  return (
    ...
    <div className="flex items-center justify-center md:mt-20 flex-wrap md:flex-nowrap">
      <div className="border-r-4 border-gray-600 border-t-4 p-4 rounded-lg shadow-lg m-10">
        <img src={product.image} alt={product.title} className='w-80 object-contain rounded-lg' />
      </div>
      <div className='max-w-xl'>
        <h1 className='text-2xl font-bold mt-4'>{product.title}</h1>
        <p className='text-gray-600 mt-2'>{product.description}</p>
        <p className='text-lg font-semibold mt-4'>${product.price}</p>
        <button className='mt-4 px-4 py-2 text-gray-700 rounded-lg'>Add to Cart</button>
        <button className='mt-4 px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white rounded-lg m-3'>Buy Now</button>
      </div>
